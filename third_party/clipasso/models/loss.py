...
class Loss(nn.Module):
    ...
    def forward(self, sketches, targets, epoch, points_optim=None, mode="train"):
        loss = 0
...
class CLIPLoss(torch.nn.Module):
    ...
    def forward(self, sketches, targets, mode="train"):
        ...
        loss_clip = (1. - torch.cosine_similarity(
            sketch_features, self.targets_features.repeat(self.NUM_AUGS, 1), dim=1))
        self.counter += 1
        return loss_clip
        # return 1. - torch.cosine_similarity(sketches_features, self.targets_features)
    ...
    def forward(self, pred, target, mode="train"):
        ...
        if mode == "train":
            for n in range(4):
                ...
                sketch_augs.append(augmented_pair[:x.size(0)])
                img_augs.append(augmented_pair[x.size(0):])
...
def l2_layers(xs_conv_features, ys_conv_features, clip_model_name):
    return [torch.square(x_conv - y_conv).view(x_conv.size(0), -1).mean(dim=1) for x_conv, y_conv in
    ...
    def forward(self, sketch, target, mode="train"):
        ...
        if mode == "train":
            ...
            for _ in range(self.num_augs):
                augmented_pair = self.augment_trans(xy)
                sketch_augs.append(augmented_pair[:x.size(0)])
                img_augs.append(augmented_pair[x.size(0):])

        xy = torch.cat(sketch_augs + img_augs, dim=0).to(self.device)
        ...
        if self.clip_model_name.startswith("RN"):
            xy_fc_features, xy_conv_features = self.forward_inspection_clip_resnet(xy)
            ...
            xy_fc_features, xy_conv_features = self.visual_encoder(xy)
            # xs_fc_features, xs_conv_features = self.visual_encoder(xs)
            # ys_fc_features, ys_conv_features = self.visual_encoder(ys)
        xs_fc_features, xs_conv_features = xy_fc_features[:xy.size(0) // 2], [f[:xy.size(0) // 2] for f in xy_conv_features]
        ys_fc_features, ys_conv_features = xy_fc_features[xy.size(0) // 2:], [f[xy.size(0) // 2:] for f in xy_conv_features]
        ...
        for layer, w in enumerate(self.args.clip_conv_layer_weights):
            if w:
                layer_conv_loss = conv_loss[layer]
                if mode == "train":
                    #print(layer_conv_loss.shape)
                    layer_conv_loss = layer_conv_loss.view(self.num_augs + 1, -1).mean(dim=0)
                conv_loss_dict[f"clip_conv_loss_layer{layer}"] = layer_conv_loss * w
        ...
        if self.clip_fc_loss_weight:
            ...
            if mode == "train":
                fc_loss = fc_loss.view(self.num_augs + 1, -1).mean(dim=0)
